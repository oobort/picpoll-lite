if (empty($_FILES['item_image']['tmp_name'])) {
        ...
        if (isset($upload['error'])) {
            ...
            echo '<div class="notice notice-error"><p>' . esc_html__('Image upload error: ', 'picpoll-lite-main') . esc_html($upload['error']) . '</p></div>';
            return;
        }
    ...
    public function admin_init_register_settings() {
        ...
        add_settings_field('regions_json', __('Regions list','picpoll-lite-main'), array($this, 'render_regions_field'), 'vg_regions', 'vg_regions');
    }
...
// Initialize plugin - no output during activation
if (!defined('ABSPATH')) {
    exit;
}

Vote_Game_Plugin::instance();<?php
/**
...
class Vote_Game_Plugin {
    ...
    public function on_activate() {
        ...
        try {
            ...
            error_log('PicPoll activation error: ' . $e->getMessage());
        }
    ...
    private function set_default_options() {
        ...
        if (empty($opts['regions_json'])) {
            $opts['regions_json'] = wp_json_encode(array(
                array('code'=>'US','label'=>'United States'),
                array('code'=>'CA','label'=>'Canada'),
                array('code'=>'UK','label'=>'United Kingdom'),
                array('code'=>'JP','label'=>'Japan'),
        ...
        if (empty($opts['option_labels_json'])) {
            $opts['option_labels_json'] = wp_json_encode(array(array('text'=>'Vote Option 1')));
            $updated = true;
        }
    ...
    private function create_tables() {
        ...
        $tables = array(
            $this->table => "CREATE TABLE {$this->table} (
            ...
            $this->adj2_table => "CREATE TABLE {$this->adj2_table} (
                image_id BIGINT UNSIGNED NOT NULL,
            ...
            $this->region_adj2_table => "CREATE TABLE {$this->region_adj2_table} (
    ...
    private function handle_add_item() {
        ...
        if (empty($_FILES['item_image']['tmp_name'])) {
            echo '<div class="notice notice-error"><p>' . esc_html__('Image is required.', 'picpoll-lite-main')        // Styling
        add_settings_section('vg_styling', __('Styling Options','picpoll-lite-main'), function(){ echo '<p>Customize the front-end appearance.</p>'; }, 'vg_settings');
        add_settings_field('accent_color', __('Accent color','picpoll-lite-main'), function(){
            $o = get_option('vg_options', array()); $val = isset($o['accent_color']) ? $o['accent_color'] : '#FF7150';
            echo '<input type="text" name="vg_options[accent_color]" value="'.esc_attr($val).'" class="regular-text" />';
